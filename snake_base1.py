    ...
    for _ in tqdm(range(training_games)):
        ...
        for _ in range(steps_per_game):
            ...
            snake_position, apple_position, score = play_game(
                snake_start, snake_position, apple_position, button_direction, score, apple_image)
            is_front_blocked, is_left_blocked, is_right_blocked = blocked_directions(snake_position)

            training_data_x.append([is_left_blocked, is_front_blocked,
                                    is_right_blocked, angle, direction])
        ...
        for _ in range(steps_per_game):
            ...
            for i in range(-1, 2):
                predictions.append(model.predict(
                    np.array([is_left_blocked, is_front_blocked, is_right_blocked, angle, i]).reshape(-1, 5, 1)))
